...
void ProcessMessageUrgentX(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "ix") {
        //LogPrintf("ProcessMessageUrgentX::ix\n");
        ...
        BOOST_FOREACH (const CTxOut o, tx.vout) {
            ...
            if (!o.scriptPubKey.IsNormalPaymentScript() && !o.scriptPubKey.IsUnspendable()) {
                LogPrintf("ProcessMessageUrgentX::ix - Invalid Script %s\n", tx.ToString().c_str());
                return;
            }
        }
        ...
        if (fAccepted) {
            ...
            LogPrintf("ProcessMessageUrgentX::ix - Transaction Lock Request: %s %s : accepted %s\n",
            ...
            LogPrintf("ProcessMessageUrgentX::ix - Transaction Lock Request: %s %s : rejected %s\n",
                pfrom->addr.ToString().c_str(), pfrom->cleanSubVer.c_str(),
                tx.GetHash().ToString().c_str());
            ...
            if (i != mapTxLocks.end()) {
                ...
                if ((*i).second.CountSignatures() >= SWIFTTX_SIGNATURES_REQUIRED) {
                    if (!CheckForConflictingLocks(tx)) {
                        LogPrintf("ProcessMessageUrgentX::ix - Found Existing Complete IX Lock\n");
    ...
    } else if (strCommand == "txlvote") //UrgentX Lock Consensus Votes
    {
        CConsensusVote ctx;
        vRecv >> ctx;
            ...
            */
            if (!mapTxLockReq.count(ctx.txHash) && !mapTxLockReqRejected.count(ctx.txHash)) {
                ...
                if (mapUnknownVotes[ctx.vinMasternode.prevout.hash] > GetTime() &&
                    ...
                    LogPrintf("ProcessMessageUrgentX::ix - masternode is spamming transaction votes: %s %s\n",
...
bool IsIXTXValid(const CTransaction& txCollateral)
{
    ...
    if (nValueOut > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
        LogPrint("urgentx", "IsIXTXValid - Transaction value too high - %s\n", txCollateral.ToString().c_str());
        return false;
    }

    if (missingTx) {
        LogPrint("urgentx", "IsIXTXValid - Unknown inputs in IX transaction - %s\n", txCollateral.ToString().c_str());
        /*
    ...
    if (nValueIn - nValueOut < COIN * 0.01) {
        LogPrint("urgentx", "IsIXTXValid - did not include enough fees in transaction %d\n%s\n", nValueOut - nValueIn, txCollateral.ToString().c_str());
        return false;
    }
...
int64_t CreateNewLock(CTransaction tx)
{
    ...
    */
    ...
    if (!mapTxLocks.count(tx.GetHash())) {
        ...
        LogPrint("urgentx", "CreateNewLock - Transaction Lock Exists %s !\n", tx.GetHash().ToString().c_str());
    }
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    if (n == -1) {
        LogPrint("urgentx", "UrgentX::DoConsensusVote - Unknown Masternode\n");
        return;
    }

    if (n > SWIFTTX_SIGNATURES_TOTAL) {
        LogPrint("urgentx", "UrgentX::DoConsensusVote - Masternode not in the top %d (%d)\n", SWIFTTX_SIGNATURES_TOTAL, n);
        return;
    }
    /*
    ...
    */
    ...
    LogPrint("urgentx", "UrgentX::DoConsensusVote - In the top %d (%d)\n", SWIFTTX_SIGNATURES_TOTAL, n);
    ...
    if (!ctx.Sign()) {
        LogPrintf("UrgentX::DoConsensusVote - Failed to sign consensus vote\n");
        return;
    }
    if (!ctx.SignatureValid()) {
        LogPrintf("UrgentX::DoConsensusVote - Signature invalid\n");
        return;
    }
...
bool ProcessConsensusVote(CNode* pnode, CConsensusVote& ctx)
{
    ...
    if (pmn != NULL)
        LogPrint("urgentx", "UrgentX::ProcessConsensusVote - Masternode ADDR %s %d\n", pmn->addr.ToString().c_str(), n);

    if (n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("urgentx", "UrgentX::ProcessConsensusVote - Unknown Masternode\n");
        mnodeman.AskForMN(pnode, ctx.vinMasternode);
        return false;
    }

    if (n > SWIFTTX_SIGNATURES_TOTAL) {
        LogPrint("urgentx", "UrgentX::ProcessConsensusVote - Masternode not in the top %d (%d) - %s\n", SWIFTTX_SIGNATURES_TOTAL, n, ctx.GetHash().ToString().c_str());
        return false;
    }

    if (!ctx.SignatureValid()) {
        LogPrintf("UrgentX::ProcessConsensusVote - Signature invalid\n");
    ...
    if (!mapTxLocks.count(ctx.txHash)) {
        LogPrintf("UrgentX::ProcessConsensusVote - New Transaction Lock %s !\n", ctx.txHash.ToString().c_str());
        ...
        LogPrint("urgentx", "UrgentX::ProcessConsensusVote - Transaction Lock Exists %s !\n", ctx.txHash.ToString().c_str());
    ...
    if (i != mapTxLocks.end()) {
        ...
        LogPrint("urgentx", "UrgentX::ProcessConsensusVote - Transaction Lock Votes %d - %s !\n", (*i).second.CountSignatures(), ctx.GetHash().ToString().c_str());

        if ((*i).second.CountSignatures() >= SWIFTTX_SIGNATURES_REQUIRED) {
            LogPrint("urgentx", "UrgentX::ProcessConsensusVote - Transaction Lock Is Complete %s !\n", (*i).second.GetHash().ToString().c_str());
    ...
    BOOST_FOREACH (const CTxIn& in, tx.vin) {
        if (mapLockedInputs.count(in.prevout)) {
            if (mapLockedInputs[in.prevout] != tx.GetHash()) {
                LogPrintf("UrgentX::CheckForConflictingLocks - found two complete conflicting locks - removing both. %s %s", tx.GetHash().ToString().c_str(), mapLockedInputs[in.prevout].ToString().c_str());
...
bool CConsensusVote::SignatureValid()
{
    ...
    if (pmn == NULL) {
        LogPrintf("UrgentX::CConsensusVote::SignatureValid() - Unknown Masternode\n");
        return false;
    }

    if (!miRageSigner.VerifyMessage(pmn->pubKeyMasternode, vchMasterNodeSignature, strMessage, errorMessage)) {
        LogPrintf("UrgentX::CConsensusVote::SignatureValid() - Verify message failed\n");
        return false;
    }
...
bool CConsensusVote::Sign()
{
    ...
    if (!miRageSigner.SignMessage(strMessage, errorMessage, vchMasterNodeSignature, key2)) {
        LogPrintf("CConsensusVote::Sign() - Sign message failed");
        return false;
    }

    if (!miRageSigner.VerifyMessage(pubkey2, vchMasterNodeSignature, strMessage, errorMessage)) {
