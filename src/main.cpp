...
#if defined(NDEBUG)
#error "YUP cannot be compiled without assertions."
#endif
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if (mapMirageBroadcastTxes.count(hash)) {
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight <= 200) {
        nSubsidy = 5000 * COIN;
    } else if (nHeight <= 21600 && nHeight > 201) {
        nSubsidy = 50 * COIN;
    } else if (nHeight <=43200  && nHeight > 21600) {
        nSubsidy = 25 * COIN;
...
/*
    ...
    Remove conflicting blocks for successful UrgentX transaction locks
...
bool static AlreadyHave(const CInv& inv)
{
    ...
    case MSG_DSTX:
        return mapMirageBroadcastTxes.count(inv.hash);
    case MSG_BLOCK:
        ...
        const CInv& inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK) {
                ...
                if (!pushed && inv.type == MSG_DSTX) {
                    if (mapMirageBroadcastTxes.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mapMirageBroadcastTxes[inv.hash].tx << mapMirageBroadcastTxes[inv.hash].vin << mapMirageBroadcastTxes[inv.hash].vchSig << mapMirageBroadcastTxes[inv.hash].sigTime;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        if (strCommand == "tx") {
            ...
            if (pmn != NULL) {
                ...
                if (!mapMirageBroadcastTxes.count(tx.GetHash())) {
                    ...
                    mapMirageBroadcastTxes.insert(make_pair(tx.GetHash(), dstx));
                }
            }
        }
    ...
    // getaddr message mitigates the attack.
        ...
        miRagePool.ProcessMessageMirage(pfrom, strCommand, vRecv);
        mnodeman.ProcessMessage(pfrom, strCommand, vRecv);
        budget.ProcessMessage(pfrom, strCommand, vRecv);
        masternodePayments.ProcessMessageMasternodePayments(pfrom, strCommand, vRecv);
        ProcessMessageUrgentX(pfrom, strCommand, vRecv);
