...
// Copyright (c) 2015-2017 The YUP developers
...
// The main object for accessing Mirage
CMiragePool miRagePool;
// A helper object for signing messages from Masternodes
CMiRageSigner miRageSigner;
// The current Mirages in progress on the network
std::vector<CMirageQueue> vecMirageQueue;
// Keep track of the used Masternodes
std::vector<CTxIn> vecMasternodesUsed;
// Keep track of the scanning errors I've seen
map<uint256, CMirageBroadcastTx> mapMirageBroadcastTxes;
// Keep track of the active Masternode
CActiveMasternode activeMasternode;

/* *** BEGIN OBFUSCATION MAGIC - YUP **********
    Copyright (c) 2014-2015, Dash Developers
        eduffield - evan@dashpay.io
        udjinm6   - udjinm6@dashpay.io
*/

void CMiragePool::ProcessMessageMirage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    if (fLiteMode) return; //disable all Mirage/Masternode related functionality
    if (!masternodeSync.IsBlockchainSynced()) return;

    if (strCommand == "dsa") { //Mirage Accept Into Pool
    ...
    } else if (strCommand == "dsq") { //Mirage Queue
        TRY_LOCK(cs_mirage, lockRecv);
        if (!lockRecv) return;
        ...
        if (dsq.ready) {
            ...
            if (state == POOL_STATUS_QUEUE) {
                LogPrint("mirage", "Mirage queue is ready - %s\n", addr.ToString());
                PrepareMirageDenominate();
            }
        } else {
            BOOST_FOREACH (CMirageQueue q, vecMirageQueue) {
                if (q.vin == dsq.vin) return;
            }

            LogPrint("mirage", "dsq last %d last2 %d count %d\n", pmn->nLastDsq, pmn->nLastDsq + mnodeman.size() / 5, mnodeman.nDsqCount);
            //don't allow a few nodes to dominate the queuing process
            if (pmn->nLastDsq != 0 &&
                pmn->nLastDsq + mnodeman.CountEnabled(ActiveProtocol()) / 5 > mnodeman.nDsqCount) {
                LogPrint("mirage", "dsq -- Masternode sending too many dsq messages. %s \n", pmn->addr.ToString());
                return;
            }
            mnodeman.nDsqCount++;
            ...
            LogPrint("mirage", "dsq - new Mirage queue object - %s\n", addr.ToString());
            vecMirageQueue.push_back(dsq);
            dsq.Relay();
            dsq.time = GetTime();
        }

    } else if (strCommand == "dsi") { //MiRage vIn
        int errorID;
        ...
        //check it like a transaction
        {
            ...
            if (nValueIn > OBFUSCATION_POOL_MAX) {
                LogPrintf("dsi -- more than Mirage pool max! %s\n", tx.ToString());
                errorID = ERR_MAXIMUM;
                pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), MASTERNODE_REJECTED, errorID);
                return;
            }
    ...
    } else if (strCommand == "dssu") { //Mirage status update
        if (pfrom->nVersion < ActiveProtocol()) {
            return;
        }
        ...
        BOOST_FOREACH (const CTxIn item, sigs) {
            ...
            LogPrint("mirage", " -- sigs count %d %d\n", (int)sigs.size(), count);
            count++;
        }

        if (success) {
            miRagePool.Check();
            RelayStatus(miRagePool.sessionID, miRagePool.GetState(), miRagePool.GetEntriesCount(), MASTERNODE_RESET);
        }
    } else if (strCommand == "dsf") { //Mirage Final tx
        if (pfrom->nVersion < ActiveProtocol()) {
            return;
        }
        ...
        if (sessionID != sessionIDMessage) {
            LogPrint("mirage", "dsf - message doesn't match current Mirage session %d %d\n", sessionID, sessionIDMessage);
            return;
        }
    ...
    } else if (strCommand == "dsc") { //Mirage Complete
        ...
        if (sessionID != sessionIDMessage) {
            LogPrint("mirage", "dsc - message doesn't match current Mirage session %d %d\n", miRagePool.sessionID, sessionIDMessage);
            return;
        }

        miRagePool.CompletedTransaction(error, errorID);
    }
}

int randomizeList(int i) { return std::rand() % i; }

void CMiragePool::Reset()
{
    cachedLastSuccess = 0;
    lastNewBlock = 0;
...
bool CMiragePool::SetCollateralAddress(std::string strAddress)
{
    CBitcoinAddress address;
    if (!address.SetString(strAddress)) {
        LogPrintf("CMiragePool::SetCollateralAddress - Invalid Mirage collateral address\n");
        return false;
    }
...
//
// Unlock coins after Mirage fails or succeeds
//
void CMiragePool::UnlockCoins()
{
    while (true) {
...
std::string CMiragePool::GetStatus()
{
    static int showingMiRageMessage = 0;
    showingMiRageMessage += 10;
    std::string suffix = "";
    ...
    case POOL_STATUS_IDLE:
        return _("Mirage is idle.");
    case POOL_STATUS_ACCEPTING_ENTRIES:
        if (entriesCount == 0) {
            showingMiRageMessage = 0;
            return strAutoDenomResult;
        } else if (lastEntryAccepted == 1) {
            if (showingMiRageMessage % 10 > 8) {
                lastEntryAccepted = 0;
                showingMiRageMessage = 0;
            }
            return _("Mirage request complete:") + " " + _("Your transaction was accepted into the pool!");
        } else {
            std::string suffix = "";
            if (showingMiRageMessage % 70 <= 40)
                return strprintf(_("Submitted following entries to masternode: %u / %d"), entriesCount, GetMaxPoolTransactions());
            else if (showingMiRageMessage % 70 <= 50)
                suffix = ".";
            else if (showingMiRageMessage % 70 <= 60)
                suffix = "..";
            else if (showingMiRageMessage % 70 <= 70)
                suffix = "...";
    ...
    case POOL_STATUS_ERROR:
        return _("Mirage request incomplete:") + " " + lastMessage + " " + _("Will retry...");
    case POOL_STATUS_SUCCESS:
        return _("Mirage request complete:") + " " + lastMessage;
    case POOL_STATUS_QUEUE:
        if (showingMiRageMessage % 70 <= 30)
            suffix = ".";
        else if (showingMiRageMessage % 70 <= 50)
            suffix = "..";
        else if (showingMiRageMessage % 70 <= 70)
...
//
// Check the Mirage progress and send client updates if a Masternode
//
void CMiragePool::Check()
{
    if (fMasterNode) LogPrint("mirage", "CMiragePool::Check() - entries count %lu\n", entries.size());
    //printf("CMiragePool::Check() %d - %d - %d\n", state, anonTx.CountEntries(), GetTimeMillis()-lastTimeChanged);

    if (fMasterNode) {
        LogPrint("mirage", "CMiragePool::Check() - entries count %lu\n", entries.size());

        // If entries is full, then move on to the next phase
        if (state == POOL_STATUS_ACCEPTING_ENTRIES && (int)entries.size() >= GetMaxPoolTransactions()) {
            LogPrint("mirage", "CMiragePool::Check() -- TRYING TRANSACTION \n");
    ...
    if (state == POOL_STATUS_FINALIZE_TRANSACTION) {
        LogPrint("mirage", "CMiragePool::Check() -- FINALIZE TRANSACTIONS\n");
        ...
        if (fMasterNode) {
            ...
            LogPrint("mirage", "Transaction 1: %s\n", txNew.ToString());
    ...
    if (fMasterNode && state == POOL_STATUS_SIGNING && SignaturesComplete()) {
        LogPrint("mirage", "CMiragePool::Check() -- SIGNING\n");
    ...
    if ((state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS) && GetTimeMillis() - lastTimeChanged >= 10000) {
        LogPrint("mirage", "CMiragePool::Check() -- timeout, RESETTING\n");
        UnlockCoins();
        SetNull();
...
void CMiragePool::CheckFinalTransaction()
{
    ...
    {
        LogPrint("mirage", "Transaction 2: %s\n", txNew.ToString());

        // See if the transaction is valid
        if (!txNew.AcceptToMemoryPool(false, true, true)) {
            LogPrintf("CMiragePool::Check() - CommitTransaction : Error: Transaction not valid\n");
            SetNull();
        ...
        LogPrintf("CMiragePool::Check() -- IS MASTER -- TRANSMITTING OBFUSCATION\n");
        ...
        if (!miRageSigner.SetKey(strMasterNodePrivKey, strError, key2, pubkey2)) {
            LogPrintf("CMiragePool::Check() - ERROR: Invalid Masternodeprivkey: '%s'\n", strError);
            return;
        }

        if (!miRageSigner.SignMessage(strMessage, strError, vchSig, key2)) {
            LogPrintf("CMiragePool::Check() - Sign message failed\n");
            return;
        }

        if (!miRageSigner.VerifyMessage(pubkey2, vchSig, strMessage, strError)) {
            LogPrintf("CMiragePool::Check() - Verify message failed\n");
            return;
        }

        if (!mapMirageBroadcastTxes.count(txNew.GetHash())) {
            CMirageBroadcastTx dstx;
            dstx.tx = txNew;
            dstx.vin = activeMasternode.vin;
            dstx.vchSig = vchSig;
            dstx.sigTime = sigTime;

            mapMirageBroadcastTxes.insert(make_pair(txNew.GetHash(), dstx));
        }
        ...
        // Reset
        LogPrint("mirage", "CMiragePool::Check() -- COMPLETED -- RESETTING\n");
        SetNull();
        RelayStatus(sessionID, GetState(), GetEntriesCount(), MASTERNODE_RESET);
    }
}
...
//
// Why bother? Mirage uses collateral to ensure abuse to the process is kept to a minimum.
// The submission and signing stages in Mirage are completely separate. In the cases where
...
void CMiragePool::ChargeFees()
{
    ...
    if (state == POOL_STATUS_ACCEPTING_ENTRIES) {
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            bool found = false;
            BOOST_FOREACH (const CMiRageEntry& v, entries) {
            ...
            if (!found) {
                LogPrintf("CMiragePool::ChargeFees -- found uncooperative node (didn't send transaction). Found offence.\n");
                offences++;
            }
        }
    }
    ...
    if (state == POOL_STATUS_SIGNING) {
        ...
        BOOST_FOREACH (const CMiRageEntry v, entries) {
            BOOST_FOREACH (const CTxDSIn s, v.sev) {
                if (!s.fHasSig) {
                    LogPrintf("CMiragePool::ChargeFees -- found uncooperative node (didn't sign). Found offence\n");
                    offences++;
                }
            }
        }
    ...
    if (state == POOL_STATUS_ACCEPTING_ENTRIES) {
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            // This queue entry didn't send us the promised transaction
            if (!found && r > target) {
                LogPrintf("CMiragePool::ChargeFees -- found uncooperative node (didn't send transaction). charging fees.\n");
                ...
                if (!wtxCollateral.AcceptToMemoryPool(true)) {
                    ...
                    LogPrintf("CMiragePool::ChargeFees() : Error: Transaction not valid");
                }
        ...
        BOOST_FOREACH (const CMiRageEntry v, entries) {
            BOOST_FOREACH (const CTxDSIn s, v.sev) {
                if (!s.fHasSig && r > target) {
                    LogPrintf("CMiragePool::ChargeFees -- found uncooperative node (didn't sign). charging fees.\n");
...
//  - Mirage is completely free, to pay miners we randomly pay the collateral of users.
void CMiragePool::ChargeRandomFees()
{
    if (fMasterNode) {
        int i = 0;
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                Being that Mirage has "no fees" we need to have some kind of cost associated
                with using it to stop abuse. Otherwise it could serve as an attack vector and
                allow endless transaction that would bloat YUP and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 YUP per transaction on average.
            */
            if (r <= 10) {
                LogPrintf("CMiragePool::ChargeRandomFees -- charging random fees. %u\n", i);
                ...
                if (!wtxCollateral.AcceptToMemoryPool(true)) {
                    // This must not fail. The transaction has already been signed and recorded.
                    LogPrintf("CMiragePool::ChargeRandomFees() : Error: Transaction not valid");
                }
                wtxCollateral.RelayWalletTransaction();
            }
        }
...
//
// Check for various timeouts (queue objects, Mirage, etc)
//
void CMiragePool::CheckTimeout()
{
    if (!fEnableMirage && !fMasterNode) return;

    // catching hanging sessions
    if (!fMasterNode) {
        switch (state) {
        case POOL_STATUS_TRANSMISSION:
            LogPrint("mirage", "CMiragePool::CheckTimeout() -- Session complete -- Running Check()\n");
            Check();
            break;
        case POOL_STATUS_ERROR:
            LogPrint("mirage", "CMiragePool::CheckTimeout() -- Pool error -- Running Check()\n");
            Check();
            break;
        case POOL_STATUS_SUCCESS:
            LogPrint("mirage", "CMiragePool::CheckTimeout() -- Pool success -- Running Check()\n");
            Check();
            break;
        }
    }

    // check Mirage queue objects for timeouts
    int c = 0;
    vector<CMirageQueue>::iterator it = vecMirageQueue.begin();
    while (it != vecMirageQueue.end()) {
        if ((*it).IsExpired()) {
            LogPrint("mirage", "CMiragePool::CheckTimeout() : Removing expired queue entry - %d\n", c);
            it = vecMirageQueue.erase(it);
        } else
            ++it;
        c++;
    }
    ...
    if (state == POOL_STATUS_ACCEPTING_ENTRIES || state == POOL_STATUS_QUEUE) {
        ...
        vector<CMiRageEntry>::iterator it2 = entries.begin();
        while (it2 != entries.end()) {
            if ((*it2).IsExpired()) {
                LogPrint("mirage", "CMiragePool::CheckTimeout() : Removing expired entry - %d\n", c);
        ...
        LogPrint("mirage", "CMiragePool::CheckTimeout() -- Session timed out (%ds) -- resetting\n", OBFUSCATION_QUEUE_TIMEOUT);
        UnlockCoins();
        SetNull();
    ...
    if (state == POOL_STATUS_SIGNING && GetTimeMillis() - lastTimeChanged >= (OBFUSCATION_SIGNING_TIMEOUT * 1000) + addLagTime) {
        LogPrint("mirage", "CMiragePool::CheckTimeout() -- Session timed out (%ds) -- restting\n", OBFUSCATION_SIGNING_TIMEOUT);
        ChargeFees();
        UnlockCoins();
        SetNull();
...
//
void CMiragePool::CheckForCompleteQueue()
{
    if (!fEnableMirage && !fMasterNode) return;
...
bool CMiragePool::SignatureValid(const CScript& newSig, const CTxIn& newVin)
{
    CMutableTransaction txNew;
    txNew.vin.clear();
    txNew.vout.clear();
    ...
    BOOST_FOREACH (CMiRageEntry& e, entries) {
    ...
    if (found >= 0) { //might have to do this one input at a time?
        ...
        LogPrint("mirage", "CMiragePool::SignatureValid() - Sign with sig %s\n", newSig.ToString().substr(0, 24));
        if (!VerifyScript(txNew.vin[n].scriptSig, sigPubKey, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRICTENC, MutableTransactionSignatureChecker(&txNew, n))) {
            LogPrint("mirage", "CMiragePool::SignatureValid() - Signing - Error signing input %u\n", n);
            return false;
        }
    }

    LogPrint("mirage", "CMiragePool::SignatureValid() - Signing - Successfully validated input\n");
    return true;
}

// check to make sure the collateral provided by the client is valid
bool CMiragePool::IsCollateralValid(const CTransaction& txCollateral)
{
    ...
    BOOST_FOREACH (const CTxOut o, txCollateral.vout) {
        ...
        if (!o.scriptPubKey.IsNormalPaymentScript()) {
            LogPrintf("CMiragePool::IsCollateralValid - Invalid Script %s\n", txCollateral.ToString());
            return false;
        }
    }
    ...
    if (missingTx) {
        LogPrint("mirage", "CMiragePool::IsCollateralValid - Unknown inputs in collateral transaction - %s\n", txCollateral.ToString());
        return false;
    }
    ...
    if (nValueIn - nValueOut < OBFUSCATION_COLLATERAL) {
        LogPrint("mirage", "CMiragePool::IsCollateralValid - did not include enough fees in transaction %d\n%s\n", nValueOut - nValueIn, txCollateral.ToString());
        return false;
    }

    LogPrint("mirage", "CMiragePool::IsCollateralValid %s\n", txCollateral.ToString());

    {
        LOCK(cs_main);
        CValidationState state;
        if (!AcceptableInputs(mempool, state, txCollateral, true, NULL)) {
            if (fDebug) LogPrintf("CMiragePool::IsCollateralValid - didn't pass IsAcceptable\n");
            return false;
        }
    }
...
//
bool CMiragePool::AddEntry(const std::vector<CTxIn>& newInput, const CAmount& nAmount, const CTransaction& txCollateral, const std::vector<CTxOut>& newOutput, int& errorID)
{
    if (!fMasterNode) return false;

    BOOST_FOREACH (CTxIn in, newInput) {
        if (in.prevout.IsNull() || nAmount < 0) {
            LogPrint("mirage", "CMiragePool::AddEntry - input not valid!\n");
    ...
    if (!IsCollateralValid(txCollateral)) {
        LogPrint("mirage", "CMiragePool::AddEntry - collateral not valid!\n");
    ...
    if ((int)entries.size() >= GetMaxPoolTransactions()) {
        LogPrint("mirage", "CMiragePool::AddEntry - entries is full!\n");
    ...
    BOOST_FOREACH (CTxIn in, newInput) {
        LogPrint("mirage", "looking for vin -- %s\n", in.ToString());
        BOOST_FOREACH (const CMiRageEntry& v, entries) {
            BOOST_FOREACH (const CTxDSIn& s, v.sev) {
                if ((CTxIn)s == in) {
                    LogPrint("mirage", "CMiragePool::AddEntry - found in vin\n");
    ...
    LogPrint("mirage", "CMiragePool::AddEntry -- adding %s\n", newInput[0].ToString());
    errorID = MSG_ENTRIES_ADDED;

    return true;
}

bool CMiragePool::AddScriptSig(const CTxIn& newVin)
{
    LogPrint("mirage", "CMiragePool::AddScriptSig -- new sig  %s\n", newVin.scriptSig.ToString().substr(0, 24));


    BOOST_FOREACH (const CMiRageEntry& v, entries) {
        BOOST_FOREACH (const CTxDSIn& s, v.sev) {
            if (s.scriptSig == newVin.scriptSig) {
                LogPrint("mirage", "CMiragePool::AddScriptSig - already exists\n");
                return false;
            }
        }
    }
    ...
    if (!SignatureValid(newVin.scriptSig, newVin)) {
        LogPrint("mirage", "CMiragePool::AddScriptSig - Invalid Sig\n");
        return false;
    }

    LogPrint("mirage", "CMiragePool::AddScriptSig -- sig %s\n", newVin.ToString());

    BOOST_FOREACH (CTxIn& vin, finalTransaction.vin) {
        if (newVin.prevout == vin.prevout && vin.nSequence == newVin.nSequence) {
            vin.scriptSig = newVin.scriptSig;
            vin.prevPubKey = newVin.prevPubKey;
            LogPrint("mirage", "CMiRagePool::AddScriptSig -- adding to finalTransaction  %s\n", newVin.scriptSig.ToString().substr(0, 24));
        }
    }
    for (unsigned int i = 0; i < entries.size(); i++) {
        if (entries[i].AddSig(newVin)) {
            LogPrint("mirage", "CMiRagePool::AddScriptSig -- adding  %s\n", newVin.scriptSig.ToString().substr(0, 24));
            return true;
        }
    }

    LogPrintf("CMiragePool::AddScriptSig -- Couldn't set sig!\n");
    return false;
}

// Check to make sure everything is signed
bool CMiragePool::SignaturesComplete()
{
    BOOST_FOREACH (const CMiRageEntry& v, entries) {
        BOOST_FOREACH (const CTxDSIn& s, v.sev) {
...
//
// Execute a Mirage denomination via a Masternode.
// This is only ran from clients
//
void CMiragePool::SendMirageDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, CAmount amount)
{
    if (fMasterNode) {
        LogPrintf("CMiragePool::SendMirageDenominate() - Mirage from a Masternode is not supported currently.\n");
        return;
    }

    if (txCollateral == CMutableTransaction()) {
        LogPrintf("CMiragePool:SendMirageDenominate() - Mirage collateral not set");
        return;
    }
    ...
    if (!sessionFoundMasternode) {
        LogPrintf("CMiragePool::SendMirageDenominate() - No Masternode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return;
    }
    ...
    if (!CheckDiskSpace()) {
        ...
        SetNull();
        fEnableMirage = false;
        LogPrintf("CMiragePool::SendMirageDenominate() - Not enough disk space, disabling Mirage.\n");
        return;
    }

    UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);

    LogPrintf("CMiragePool::SendMirageDenominate() - Added transaction to pool.\n");
...
// Incoming message from Masternode updating the progress of Mirage
//    newAccepted:  -1 mean's it'n not a "transaction accepted/not accepted" message, just a standard update
//                  0 means transaction was not accepted
//                  1 means transaction was accepted

bool CMiragePool::StatusUpdate(int newState, int newEntriesCount, int newAccepted, int& errorID, int newSessionID)
{
    if (fMasterNode) return false;
    ...
    if (newAccepted != -1) {
        ...
        if (newAccepted == 1 && newSessionID != 0) {
            ...
            LogPrintf("CMiragePool::StatusUpdate - set sessionID to %d\n", sessionID);
    ...
    if (newState == POOL_STATUS_ACCEPTING_ENTRIES) {
        if (newAccepted == 1) {
            LogPrintf("CMiragePool::StatusUpdate - entry accepted! \n");
            sessionFoundMasternode = true;
            //wait for other users. Masternode will report when ready
            UpdateState(POOL_STATUS_QUEUE);
        } else if (newAccepted == 0 && sessionID == 0 && !sessionFoundMasternode) {
            LogPrintf("CMiragePool::StatusUpdate - entry not accepted by Masternode \n");
            UnlockCoins();
            UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);
...
//
bool CMiragePool::SignFinalTransaction(CTransaction& finalTransactionNew, CNode* node)
{
    if (fMasterNode) return false;

    finalTransaction = finalTransactionNew;
    LogPrintf("CMiragePool::SignFinalTransaction %s", finalTransaction.ToString());

    vector<CTxIn> sigs;

    //make sure my inputs/outputs are present, otherwise refuse to sign
    BOOST_FOREACH (const CMiRageEntry e, entries) {
        BOOST_FOREACH (const CTxDSIn s, e.sev) {
            ...
            if (mine >= 0) { //might have to do this one input at a time?
                ...
                if (foundOutputs < targetOuputs || nValue1 != nValue2) {
                    ...
                    LogPrintf("CMiragePool::Sign - My entries are not correct! Refusing to sign. %d entries %d target. \n", foundOutputs, targetOuputs);
                    UnlockCoins();
                    SetNull();
...
void CMiragePool::NewBlock()
{
    LogPrint("mirage", "CMiragePool::NewBlock \n");

    //we we're processing lots of blocks, we'll just leave
    if (GetTime() - lastNewBlock < 10) return;
    lastNewBlock = GetTime();

    miRagePool.CheckTimeout();
}

// Mirage transaction was completed (failed or successful)
void CMiragePool::CompletedTransaction(bool error, int errorID)
{
    if (fMasterNode) return;
...
void CMiragePool::ClearLastMessage()
{
    lastMessage = "";
}

//
// Passively run Mirage in the background to anonymize funds based on the given configuration.
//
// This does NOT run by default for daemons, only for QT.
//
bool CMiragePool::DoAutomaticDenominating(bool fDryRun)
{
    if (!fEnableMirage) return false;
    if (fMasterNode) return false;
    if (state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS) return false;
    ...
    if (chainActive.Tip()->nHeight - cachedLastSuccess < minBlockSpacing) {
        LogPrintf("CMiragePool::DoAutomaticDenominating - Last successful Mirage action was too recent\n");
        strAutoDenomResult = _("Last successful Mirage action was too recent.");
        return false;
    }

    if (mnodeman.size() == 0) {
        LogPrint("mirage", "CMiragePool::DoAutomaticDenominating - No Masternodes detected\n");
    ...
    CAmount nLowestDenom = OBFUSCATION_COLLATERAL + miRageDenominations[miRageDenominations.size() - 1] * 5;
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeYupAmount * COIN - pwalletMain->GetAnonymizedBalance();
    ...
    LogPrint("mirage", "DoAutomaticDenominating : nLowestDenom=%d, nBalanceNeedsAnonymized=%d\n", nLowestDenom, nBalanceNeedsAnonymized);

    // select coins that should be given to the pool
    if (!pwalletMain->SelectCoinsDark(nValueMin, nBalanceNeedsAnonymized, vCoins, nValueIn, 0, nMirageRounds)) {
        nValueIn = 0;
        vCoins.clear();
    ...
    if (!sessionFoundMasternode) {
        ...
        if (nUseQueue > 33) {
            ...
            BOOST_FOREACH (CMirageQueue& dsq, vecMirageQueue) {
                ...
                if (!pwalletMain->SelectCoinsByDenominations(dsq.nDenom, nValueMin, nBalanceNeedsAnonymized, vTempCoins, vTempCoins2, nValueIn, 0, nMirageRounds)) {
...
bool CMiragePool::PrepareMirageDenominate()
{
    std::string strError = "";
    // Submit transaction to the pool if we get here
    // Try to use only inputs with the same number of rounds starting from lowest number of rounds possible
    for (int i = 0; i < nMirageRounds; i++) {
        strError = pwalletMain->PrepareMirageDenominate(i, i + 1);
        LogPrintf("DoAutomaticDenominating : Running Mirage denominate for %d rounds. Return '%s'\n", i, strError);
        if (strError == "") return true;
    }

    // We failed? That's strange but let's just make final attempt and try to mix everything
    strError = pwalletMain->PrepareMirageDenominate(0, nMirageRounds);
    LogPrintf("DoAutomaticDenominating : Running Mirage denominate for all rounds. Return '%s'\n", strError);
    if (strError == "") return true;
...
bool CMiragePool::SendRandomPaymentToSelf()
{
...
bool CMiragePool::MakeCollateralAmounts()
{
    CWalletTx wtx;
    CAmount nFeeRet = 0;
    std::string strFail = "";
...
bool CMiragePool::CreateDenominated(CAmount nTotalValue)
{
    CWalletTx wtx;
    CAmount nFeeRet = 0;
    std::string strFail = "";
    ...
    // ****** Add denoms ************ /
    BOOST_REVERSE_FOREACH (CAmount v, miRageDenominations) {
        int nOutputs = 0;
...
bool CMiragePool::IsCompatibleWithEntries(std::vector<CTxOut>& vout)
{
...
bool CMiragePool::IsCompatibleWithSession(int64_t nDenom, CTransaction txCollateral, int& errorID)
{
    if (nDenom == 0) return false;

    LogPrintf("CMiragePool::IsCompatibleWithSession - sessionDenom %d sessionUsers %d\n", sessionDenom, sessionUsers);

    if (!unitTest && !IsCollateralValid(txCollateral)) {
        LogPrint("mirage", "CMiragePool::IsCompatibleWithSession - collateral not valid!\n");
        errorID = ERR_INVALID_COLLATERAL;
        return false;
    }
    ...
    if ((state != POOL_STATUS_ACCEPTING_ENTRIES && state != POOL_STATUS_QUEUE) || sessionUsers >= GetMaxPoolTransactions()) {
        ...
        LogPrintf("CMiragePool::IsCompatibleWithSession - incompatible mode, return false %d %d\n", state != POOL_STATUS_ACCEPTING_ENTRIES, sessionUsers >= GetMaxPoolTransactions());
        return false;
    }
    ...
    LogPrintf("CMiRagePool::IsCompatibleWithSession - compatible\n");
...
void CMiragePool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    // Function returns as follows:
    //
    // bit 0 - 100YUP+1 ( bit on if present )
    // bit 1 - 10YUP+1
    // bit 2 - 1YUP+1
    // bit 3 - .1YUP+1
    // bit 3 - non-denom
...
int CMiragePool::GetDenominations(const std::vector<CTxDSOut>& vout)
{
...
int CMiragePool::GetDenominations(const std::vector<CTxOut>& vout, bool fSingleRandomDenom)
{
    std::vector<pair<int64_t, int> > denomUsed;

    // make a list of denominations, with zero uses
    BOOST_FOREACH (int64_t d, miRageDenominations)
...
int CMiragePool::GetDenominationsByAmounts(std::vector<CAmount>& vecAmount)
{
    CScript e = CScript();
...
int CMiragePool::GetDenominationsByAmount(CAmount nAmount, int nDenomTarget)
{
    CScript e = CScript();
...
std::string CMiragePool::GetMessageByID(int messageID)
{
    switch (messageID) {
    ...
    case ERR_MAXIMUM:
        return _("Value more than Mirage pool maximum allows.");
    case ERR_MN_LIST:
    ...
    case ERR_RECENT:
        return _("Last Mirage was too recent.");
    case ERR_SESSION:
...
bool CMiRageSigner::IsVinAssociatedWithPubkey(CTxIn& vin, CPubKey& pubkey)
{
    CScript payee2;
...
bool CMiRageSigner::SetKey(std::string strSecret, std::string& errorMessage, CKey& key, CPubKey& pubkey)
{
...
bool CMiRageSigner::GetKeysFromSecret(std::string strSecret, CKey& keyRet, CPubKey& pubkeyRet)
{
    CBitcoinSecret vchSecret;
...
bool CMiRageSigner::SignMessage(std::string strMessage, std::string& errorMessage, vector<unsigned char>& vchSig, CKey key)
{
...
bool CMiRageSigner::VerifyMessage(CPubKey pubkey, vector<unsigned char>& vchSig, std::string strMessage, std::string& errorMessage)
{
    CHashWriter ss(SER_GETHASH, 0);
    ss << strMessageMagic;
    ss << strMessage;
    ...
    if (fDebug && pubkey2.GetID() != pubkey.GetID())
        LogPrintf("CMiRageSigner::VerifyMessage -- keys don't match: %s %s\n", pubkey2.GetID().ToString(), pubkey.GetID().ToString());

    return (pubkey2.GetID() == pubkey.GetID());
}

bool CMirageQueue::Sign()
{
    if (!fMasterNode) return false;
    ...
    if (!miRageSigner.SetKey(strMasterNodePrivKey, errorMessage, key2, pubkey2)) {
        LogPrintf("CMirageQueue():Relay - ERROR: Invalid Masternodeprivkey: '%s'\n", errorMessage);
        return false;
    }

    if (!miRageSigner.SignMessage(strMessage, errorMessage, vchSig, key2)) {
        LogPrintf("CMirageQueue():Relay - Sign message failed");
        return false;
    }

    if (!miRageSigner.VerifyMessage(pubkey2, vchSig, strMessage, errorMessage)) {
        LogPrintf("CMirageQueue():Relay - Verify message failed");
        return false;
    }
...
bool CMirageQueue::CheckSignature()
{
    ...
    if (pmn != NULL) {
        ...
        std::string errorMessage = "";
        if (!miRageSigner.VerifyMessage(pmn->pubKeyMasternode, vchSig, strMessage, errorMessage)) {
            return error("CMirageQueue::CheckSignature() - Got bad Masternode address signature %s \n", vin.ToString().c_str());
        }
...
void CMiragePool::RelayFinalTransaction(const int sessionID, const CTransaction& txNew)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH (CNode* pnode, vNodes) {
...
void CMiragePool::RelayIn(const std::vector<CTxDSIn>& vin, const int64_t& nAmount, const CTransaction& txCollateral, const std::vector<CTxDSOut>& vout)
{
    if (!pSubmittedToMasternode) return;
...
void CMiragePool::RelayStatus(const int sessionID, const int newState, const int newEntriesCount, const int newAccepted, const int errorID)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH (CNode* pnode, vNodes)
...
void CMiragePool::RelayCompletedTransaction(const int sessionID, const bool error, const int errorID)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH (CNode* pnode, vNodes)
...
void ThreadCheckMiRagePool()
{
    if (fLiteMode) return; //disable all Mirage/Masternode related functionality

    // Make this thread recognisable as the wallet flushing thread
    RenameThread("yup-mirage");

    unsigned int c = 0;

    while (true) {
        MilliSleep(1000);
        //LogPrintf("ThreadCheckMiRagePool::check timeout\n");
        ...
        if (masternodeSync.IsBlockchainSynced()) {
            ...
            miRagePool.CheckTimeout();
            miRagePool.CheckForCompleteQueue();

            if (miRagePool.GetState() == POOL_STATUS_IDLE && c % 15 == 0) {
                miRagePool.DoAutomaticDenominating();
            }
        }
    }
}
