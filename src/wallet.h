...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 YUP at the same time
...
public:
    ...
    bool SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nMirageRoundsMin, int nMirageRoundsMax) const;
    bool SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nMirageRoundsMin, int nMirageRoundsMax);
    ...
    std::string PrepareMirageDenominate(int minRounds, int maxRounds);
    int GenerateMirageOutputs(int nTotalValue, std::vector<CTxOut>& vout);
    ...
    // get the Mirage chain depth for a given input
    int GetRealInputMirageRounds(CTxIn in, int rounds) const;
    // respect current settings
    int GetInputMirageRounds(CTxIn in) const;
...
public:
    ...
    CAmount GetAnonymizableCredit(bool fUseCache = true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            const int rounds = pwallet->GetInputMirageRounds(vin);
            if (rounds >= -2 && rounds < nMirageRounds) {
                nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
                if (!MoneyRange(nCredit))
...
public:
    ...
    //Used with Mirage. Will return largest nondenom, then denominations, then very small inputs
    int Priority() const
    {
