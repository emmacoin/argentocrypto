...
// Recursively determine the rounds of a given input (How deep is the Mirage chain for a given input)
int CWallet::GetRealInputMirageRounds(CTxIn in, int rounds) const
{
    ...
    if (wtx != NULL) {
        ...
        if (mdwi == mDenomWtxes.end()) {
            LogPrint("mirage", "GetInputMirageRounds INSERTING %s\n", hash.ToString());
        ...
        if (nout >= wtx->vout.size()) {
            ...
            LogPrint("mirage", "GetInputMirageRounds UPDATED   %s %3d %3d\n", hash.ToString(), nout, -4);
            return -4;
        }
        ...
        BOOST_FOREACH (CTxIn in2, wtx->vin) {
            if (IsMine(in2)) {
                int n = GetRealInputMirageRounds(in2, rounds + 1);
...
int CWallet::GetInputMirageRounds(CTxIn in) const
{
    LOCK(cs_wallet);
    int realMirageRounds = GetRealInputMirageRounds(in, 0);
    return realMirageRounds > nMirageRounds ? nMirageRounds : realMirageRounds;
}
...
{
    BOOST_FOREACH (CAmount d, miRageDenominations)
        if (nInputAmount == d)
            return true;
    return false;
}
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                int rounds = GetInputMirageRounds(vin);
                fTotal += (float)rounds;
                fCount += 1;
            }
        }
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                int rounds = GetInputMirageRounds(vin);
                nTotal += pcoin->vout[i].nValue * rounds / nMirageRounds;
            }
        }
    }
...
bool less_then_denom(const COutput& out1, const COutput& out2)
{
    ...
    bool found1 = false;
    bool found2 = false;
    BOOST_FOREACH (CAmount d, miRageDenominations) // loop through predefined denoms
    {
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    if (coinControl && coinControl->HasSelected()) {
        BOOST_FOREACH (const COutput& out, vCoins) {
            ...
            if (coin_type == ONLY_DENOMINATED) {
                ...
                if (rounds < nMirageRounds) continue;
            }
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 YUP
    if (coin_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH (CAmount v, miRageDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 YUP over
                    ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputMirageRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nMirageRoundsMin, int nMirageRoundsMax)
{
    vCoinsRet.clear();
    nValueRet = 0;
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            //
            // bit 0 - 10000 YUP+1 ( bit on if present )
            // bit 1 - 1000 YUP+1
            // bit 2 - 100 YUP+1
            // bit 3 - 10 YUP+1
            // bit 4 - 1 YUP+1
            ...
            int rounds = GetInputMirageRounds(vin);
            if (rounds >= nMirageRoundsMax) continue;
            if (rounds < nMirageRoundsMin) continue;
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nMirageRoundsMin, int nMirageRoundsMax) const
{
    ...
    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, nMirageRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT10000IFMN : ONLY_DENOMINATED);
...
bool CWallet::CreateCollateralTransaction(CMutableTransaction& txCollateral, std::string& strReason)
{
    ...
    */
    ...
    if (!SelectCoinsCollateral(vCoinsCollateral, nValueIn2)) {
        strReason = "Error: Mirage requires a collateral transaction and could not locate an acceptable input!";
        return false;
    }
    ...
    BOOST_FOREACH (CTxIn v, txCollateral.vin) {
        if (!SignSignature(*this, v.prevPubKey, txCollateral, vinNumber, int(SIGHASH_ALL | SIGHASH_ANYONECANPAY))) {
            ...
            strReason = "CMiragePool::Sign - Unable to sign collateral transaction! \n";
            return false;
        }
        vinNumber++;
    }
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 YUP.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Mirage non-denominated funds for this transaction that are not equal 10000 YUP.");
                    } else {
                        strFailReason = _("Unable to locate enough Mirage denominated funds for this transaction.");
                        strFailReason += " " + _("Mirage uses exact denominated amounts to send funds, you might simply need to anonymize some more coins.");
                    }

                    if (useIX) {
                        strFailReason += " " + _("UrgentX requires inputs with at least 6 confirmations, you might need to wait a few minutes and try again.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-yup-address
                    CScript scriptChange;
...
string CWallet::PrepareMirageDenominate(int minRounds, int maxRounds)
{
    if (IsLocked())
        return _("Error: Wallet locked, unable to create transaction!");

    if (miRagePool.GetState() != POOL_STATUS_ERROR && miRagePool.GetState() != POOL_STATUS_SUCCESS)
        if (miRagePool.GetEntriesCount() > 0)
            return _("Error: You already have pending entries in the Mirage pool");
    ...
    */
    if (minRounds >= 0) {
        if (!SelectCoinsByDenominations(miRagePool.sessionDenom, 0.1 * COIN, OBFUSCATION_POOL_MAX, vCoins, vCoins2, nValueIn, minRounds, maxRounds))
            return _("Error: Can't select current denominated inputs");
    }

    LogPrintf("PrepareMirageDenominate - preparing mirage denominate . Got: %d \n", nValueIn);
    ...
    */
    ...
    while (nStep < nStepsMax) {
        BOOST_FOREACH (CAmount v, miRageDenominations) {
            ...
            bool fAccepted = false;
            if ((miRagePool.sessionDenom & (1 << 0)) && v == ((10000 * COIN) + 10000000)) {
                fAccepted = true;
            } else if ((miRagePool.sessionDenom & (1 << 1)) && v == ((1000 * COIN) + 1000000)) {
                fAccepted = true;
            } else if ((miRagePool.sessionDenom & (1 << 2)) && v == ((100 * COIN) + 100000)) {
                fAccepted = true;
            } else if ((miRagePool.sessionDenom & (1 << 3)) && v == ((10 * COIN) + 10000)) {
                fAccepted = true;
            } else if ((miRagePool.sessionDenom & (1 << 4)) && v == ((1 * COIN) + 1000)) {
                fAccepted = true;
            } else if ((miRagePool.sessionDenom & (1 << 5)) && v == ((.1 * COIN) + 100)) {
                fAccepted = true;
            }
            if (!fAccepted) continue;
    ...
    if (miRagePool.GetDenominations(vOut) != miRagePool.sessionDenom) {
    ...
    miRagePool.SendMirageDenominate(vCoinsResult, vOut, nValueIn - nValueLeft);
