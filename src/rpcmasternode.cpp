...
Value mirage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "mirage <yupaddress> <amount>\n"
            "yupaddress, reset, or auto (AutoDenominate)"
    ...
    if (params[0].get_str() == "auto") {
        if (fMasterNode)
            return "MiRage is not supported from masternodes";

        return "DoAutomaticDenominating " + (miRagePool.DoAutomaticDenominating() ? "successful" : ("failed: " + miRagePool.GetStatus()));
    }

    if (params[0].get_str() == "reset") {
        miRagePool.Reset();
        return "successfully reset mirage";
    }

    if (params.size() != 2)
        throw runtime_error(
            "mirage <yupaddress> <amount>\n"
            "yupaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and will be rounded to the next 0.1" +
            HelpRequiringPassphrase());
...
Value getpoolinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"current\": \"addr\",    (string) YUP address of current masternode\n"
    ...
    obj.push_back(Pair("state", miRagePool.GetState()));
    obj.push_back(Pair("entries", miRagePool.GetEntriesCount()));
    obj.push_back(Pair("entries_accepted", miRagePool.GetCountEntriesAccepted()));
    return obj;
}
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  start        - Start masternode configured in yup.conf\n"
...
Value listmasternodes(const Array& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() > 1))
        throw runtime_error(
            ...
            "    \"addr\": \"addr\",      (string) Masternode YUP address\n"
...
Value masternodeconnect(const Array& params, bool fHelp)
{
    if (fHelp || (params.size() != 1))
        throw runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("masternodeconnect", "\"192.168.0.6:1237\"") + HelpExampleRpc("masternodeconnect", "\"192.168.0.6:1237\""));
...
Value getmasternodecount (const Array& params, bool fHelp)
{
    if (fHelp || (params.size() > 0))
        throw runtime_error(
            ...
            "  \"miragecompat\": n,    (numeric) Mirage Compatible\n"
...
Value getmasternodestatus (const Array& params, bool fHelp)
{
    if (fHelp || (params.size() != 0))
        throw runtime_error(
            ...
            "  \"addr\": \"xxxx\",        (string) YUP address for masternode payments\n"
            "  \"status\": \"xxxx\",      (string) Masternode status\n"
...
Value getmasternodewinners (const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"winner\": {\n"
            "      \"address\": \"xxxx\",    (string) YUP MN Address\n"
