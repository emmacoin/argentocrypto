...
void WalletModel::pollBalanceChanged()
{
    ...
    if (fForceCheckBalanceChanged || chainActive.Height() != cachedNumBlocks || nMirageRounds != cachedMirageRounds || cachedTxLocks != nCompleteTXLocks) {
        fForceCheckBalanceChanged = false;

        // Balance and number of transactions might have changed
        cachedNumBlocks = chainActive.Height();
        cachedMirageRounds = nMirageRounds;
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered yup address / amount:
...

    {
        ...
        if (recipients[0].useUrgentX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("UrgentX doesn't support sending values that high yet. Transactions are currently limited to %1 YUP.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
                CClientUIInterface::MSG_ERROR);
            return TransactionCreationFailed;
        }

        bool fCreated = wallet->CreateTransaction(vecSend, *newTx, *keyChange, nFeeRequired, strFailReason, coinControl, recipients[0].inputType, recipients[0].useUrgentX);
        transaction.setTransactionFee(nFeeRequired);

        if (recipients[0].useUrgentX && newTx->GetValueOut() > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("UrgentX doesn't support sending values that high yet. Transactions are currently limited to %1 YUP.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
                CClientUIInterface::MSG_ERROR);
            return TransactionCreationFailed;
        }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal yup:URI (yup:XyZ...?message=example)
            {
        ...
        if (!wallet->CommitTransaction(*newTx, *keyChange, (recipients[0].useUrgentX) ? "ix" : "tx"))
