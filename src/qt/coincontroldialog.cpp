...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    if (nQuantity > 0) {
        ...
        // IX Fee
        if (coinControl->useUrgentX) nPayFee = max(nPayFee, CENT);
        // Allow free?
        ...
        if (nPayAmount > 0) {
            ...
            // DS Fee = overpay
            if (coinControl->useMiRage && nChange > 0) {
                nPayFee += nChange;
                nChange = 0;
            }
    ...
    int nDisplayUnit = BitcoinUnits::YUP;
    ...
    BOOST_FOREACH (PAIRTYPE(QString, vector<COutput>) coins, mapCoins) {
        ...
        BOOST_FOREACH (const COutput& out, coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show YUP address. In tree mode, address is not shown again for direct wallet address outputs
