                                                    ...
                                                    fFeeMinimized(true)
{
    ...
    // YUP specific
    QSettings settings;
    if (!settings.contains("bUseMiRage"))
        settings.setValue("bUseMiRage", false);
    if (!settings.contains("bUseUrgentX"))
        settings.setValue("bUseUrgentX", false);

    bool useMiRage = settings.value("bUseMiRage").toBool();
    bool useUrgentX = settings.value("bUseUrgentX").toBool();
    if (fLiteMode) {
        ui->checkUseMirage->setChecked(false);
        ui->checkUseMirage->setVisible(false);
        ui->checkUrgentX->setVisible(false);
        CoinControlDialog::coinControl->useMiRage = false;
        CoinControlDialog::coinControl->useUrgentX = false;
    } else {
        ui->checkUseMirage->setChecked(useMiRage);
        ui->checkUrgentX->setChecked(useUrgentX);
        CoinControlDialog::coinControl->useMiRage = useMiRage;
        CoinControlDialog::coinControl->useUrgentX = useUrgentX;
    }

    connect(ui->checkUseMirage, SIGNAL(stateChanged(int)), this, SLOT(updateDisplayUnit()));
    connect(ui->checkUrgentX, SIGNAL(stateChanged(int)), this, SLOT(updateUrgentX()));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if (ui->checkUseMirage->isChecked()) {
        recipients[0].inputType = ONLY_DENOMINATED;
        ...
        strFee = QString(tr(
            "(mirage requires this amount to be rounded up to the nearest %1).")
    ...
    if (ui->checkUrgentX->isChecked()) {
        recipients[0].useUrgentX = true;
        strFunds += " ";
        strFunds += tr("and UrgentX");
    } else {
        recipients[0].useUrgentX = false;
    }
...
void SendCoinsDialog::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchBalance, const CAmount& watchUnconfirmedBalance, const CAmount& watchImmatureBalance)
{
    ...
    if (model && model->getOptionsModel()) {
        uint64_t bal = 0;
        QSettings settings;
        settings.setValue("bUseMiRage", ui->checkUseMirage->isChecked());
        if (ui->checkUseMirage->isChecked()) {
...
void SendCoinsDialog::updateDisplayUnit()
{
    ...
    setBalance(model->getBalance(), model->getUnconfirmedBalance(), model->getImmatureBalance(), model->getAnonymizedBalance(),
        model->getWatchBalance(), model->getWatchUnconfirmedBalance(), model->getWatchImmatureBalance());
    CoinControlDialog::coinControl->useMiRage = ui->checkUseMirage->isChecked();
    coinControlUpdateLabels();
...
void SendCoinsDialog::updateUrgentX()
{
    QSettings settings;
    settings.setValue("bUseUrgentX", ui->checkUrgentX->isChecked());
    CoinControlDialog::coinControl->useUrgentX = ui->checkUrgentX->isChecked();
    coinControlUpdateLabels();
}
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel()) {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid YUP address"));
        } else // Valid address
        {
            CPubKey pubkey;
            CKeyID keyid;
...
void SendCoinsDialog::coinControlUpdateLabels()
{
    ...
    ui->checkUseMirage->setChecked(CoinControlDialog::coinControl->useMiRage);
