...
void CMasternodeMan::ProcessMasternodeConnections()
{
    ...
    BOOST_FOREACH (CNode* pnode, vNodes) {
        if (pnode->fMiRageMaster) {
            if (miRagePool.pSubmittedToMasternode != NULL && pnode->addr == miRagePool.pSubmittedToMasternode->addr) continue;
            LogPrintf("Closing Masternode connection peer=%i \n", pnode->GetId());
            pnode->fMiRageMaster = false;
            pnode->Release();
        }
    }
}
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mnb") { //Masternode Broadcast
        ...
        if (!miRageSigner.IsVinAssociatedWithPubkey(mnb.vin, mnb.pubKeyCollateralAddress)) {
        ...
        //  - this is checked later by .check() in many places and by ThreadCheckMiRagePool()
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
    else if (strCommand == "dsee") { //MiRage Election Entry
        ...
        if (!miRageSigner.VerifyMessage(pubkey, vchSig, strMessage, errorMessage)) {
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (addr.GetPort() != 1237) return;
        } else if (addr.GetPort() == 1237)
            return;
        ...
        // make sure the vout that was signed is related to the transaction that spawned the Masternode
        //  - this is expensive, so it's only done once per Masternode
        if (!miRageSigner.IsVinAssociatedWithPubkey(vin, pubkey)) {
        ...
        CTxOut vout = CTxOut(9999.99 * COIN, miRagePool.collateralPubKey);
        ...
        if (fAcceptable) {
            ...
            // should be at least not earlier than block when 1000 YUP tx got MASTERNODE_MIN_CONFIRMATIONS
            uint256 hashBlock = 0;
            CTransaction tx2;
            GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
            BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
            if (mi != mapBlockIndex.end() && (*mi).second) {
                CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 10000 YUP tx -> 1 confirmation
    ...
    else if (strCommand == "dseep") { //MiRage Election Entry Ping
