...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "yup.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "yupd.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 1237, 1239));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in YUP/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in YUP/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-zmqpubhashtxlock=<address>", _("Enable publish hash transaction (locked via UrgentX) in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawblock=<address>", _("Enable publish raw block in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtx=<address>", _("Enable publish raw transaction in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtxlock=<address>", _("Enable publish raw transaction (locked via UrgentX) in <address>"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, yup, (mirage, urgentx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in YUP/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all YUP specific functionality (Masternodes, Mirage, UrgentX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:1237"));
    strUsage += HelpMessageOpt("-budgetvotemode=<mode>", _("Change automatic finalized budget voting behavior. mode=auto: Vote for only exact finalized budget match to my generated budget. (string, default: auto)"));

    strUsage += HelpMessageGroup(_("Mirage options:"));
    strUsage += HelpMessageOpt("-enablemirage=<n>", strprintf(_("Enable use of automated mirage for funds stored in this wallet (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-miragerounds=<n>", strprintf(_("Use N separate masternodes to anonymize funds  (2-8, default: %u)"), 2));
    strUsage += HelpMessageOpt("-anonymizeyupamount=<n>", strprintf(_("Keep N YUP anonymized (default: %u)"), 0));
    strUsage += HelpMessageOpt("-liquidityprovider=<n>", strprintf(_("Provide liquidity to Mirage by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0));

    strUsage += HelpMessageGroup(_("UrgentX options:"));
    strUsage += HelpMessageOpt("-enableurgentx=<n>", strprintf(_("Enable urgentx, show confirmations for locked transactions (bool, default: %s)"), "true"));
    strUsage += HelpMessageOpt("-urgentxdepth=<n>", strprintf(_("Show N confirmations for a successfully locked transaction (0-9999, default: %u)"), nUrgentXDepth));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 1238, 1240));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The YUP Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that YUP is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!GetBoolArg("-enableurgentx", fEnableUrgentX)) {
        if (SoftSetArg("-urgentxdepth", 0))
            LogPrintf("AppInit2 : parameter interaction: -enableurgentx=false -> setting -nUrgentXDepth=0\n");
    }
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. YUP Core is shutting down."));
...
#endif
    // Make sure only a single YUP process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. YUP Core is probably already running."), strDataDir));
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of YUP Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart YUP Core to complete") << "\n";
    ...
    if (fMasterNode) {
        ...
        if (!strMasterNodePrivKey.empty()) {
            ...
            if (!miRageSigner.SetKey(strMasterNodePrivKey, errorMessage, key, pubkey)) {
    ...
    fEnableMirage = GetBoolArg("-enablemirage", false);

    nMirageRounds = GetArg("-miragerounds", 2);
    if (nMirageRounds > 16) nMirageRounds = 16;
    if (nMirageRounds < 1) nMirageRounds = 1;

    nLiquidityProvider = GetArg("-liquidityprovider", 0); //0-100
    if (nLiquidityProvider != 0) {
        miRagePool.SetMinBlockSpacing(std::min(nLiquidityProvider, 100) * 15);
        fEnableMirage = true;
        nMirageRounds = 99999;
    }

    nAnonymizeYupAmount = GetArg("-anonymizeyupamount", 0);
    if (nAnonymizeYupAmount > 999999) nAnonymizeYupAmount = 999999;
    if (nAnonymizeYupAmount < 2) nAnonymizeYupAmount = 2;

    fEnableUrgentX = GetBoolArg("-enableurgentx", fEnableUrgentX);
    nUrgentXDepth = GetArg("-urgentxdepth", nUrgentXDepth);
    nUrgentXDepth = std::min(std::max(nUrgentXDepth, 0), 60);

    //lite mode disables all Masternode and Mirage related functionality
    ...
    LogPrintf("nUrgentXDepth %d\n", nUrgentXDepth);
    LogPrintf("Mirage rounds %d\n", nMirageRounds);
    LogPrintf("Anonymize YUP Amount %d\n", nAnonymizeYupAmount);
    LogPrintf("Budget Mode %s\n", strBudgetMode.c_str());

    /* Denominations

       A note about convertability. Within Mirage pools, each denomination
       is convertable to another.

       For example:
       1YUP+1000 == (.1YUP+100)*10
       10YUP+10000 == (1YUP+1000)*10
    */
    miRageDenominations.push_back((10000 * COIN) + 10000000);
    miRageDenominations.push_back((1000 * COIN) + 1000000);
    miRageDenominations.push_back((100 * COIN) + 100000);
    miRageDenominations.push_back((10 * COIN) + 10000);
    miRageDenominations.push_back((1 * COIN) + 1000);
    miRageDenominations.push_back((.1 * COIN) + 100);
    /* Disabled till we need them
    miRageDenominations.push_back( (.01      * COIN)+10 );
    miRageDenominations.push_back( (.001     * COIN)+1 );
    */

    miRagePool.InitCollateralAddress();

    threadGroup.create_thread(boost::bind(&ThreadCheckMiRagePool));
