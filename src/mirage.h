...
extern CMiragePool miRagePool;
extern CMiRageSigner miRageSigner;
extern std::vector<CMirageQueue> vecMirageQueue;
extern std::string strMasterNodePrivKey;
extern map<uint256, CMirageBroadcastTx> mapMirageBroadcastTxes;
extern CActiveMasternode activeMasternode;

/** Holds an Mirage input
 */
...
// A clients transaction in the mirage pool
class CMiRageEntry
{
public:
    bool isSet;
...
/**
 * A currently inprogress Mirage merge and denomination information
 */
class CMirageQueue
{
public:
...
/** Helper class to store Mirage transaction (tx) information.
 */
class CMirageBroadcastTx
{
public:
...
{
public:
    /// Is the inputs associated with this public key? (and there is 10000 YUP - checking if valid masternode)
...
/** Used to keep track of current status of Mirage pool
 */
class CMiragePool
{
private:
    mutable CCriticalSection cs_mirage;

    std::vector<CMiRageEntry> entries; // Masternode/clients entries
...
public:
    ...
    CMiragePool()
    {
        /* Mirage uses collateral addresses to trust parties entering the pool
    ...
    /** Process a Mirage message using the Mirage protocol
     * \param pfrom
     * \param strCommand lower case command string; valid values are:
     *        Command  | Description
     *        -------- | -----------------
     *        dsa      | Mirage Acceptable
     *        dsc      | Mirage Complete
     *        dsf      | Mirage Final tx
     *        dsi      | Mirage vIn
     *        dsq      | Mirage Queue
     *        dss      | Mirage Signal Final Tx
     *        dssu     | Mirage status update
     *        dssub    | Mirage Subscribe To
     * \param vRecv
     */
    void ProcessMessageMirage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv);

    void InitCollateralAddress()
    {
        SetCollateralAddress(Params().MiragePoolDummyAddress());
    }
    ...
    void UpdateState(unsigned int newState)
    {
        if (fMasterNode && (newState == POOL_STATUS_ERROR || newState == POOL_STATUS_SUCCESS)) {
            LogPrint("mirage", "CMiragePool::UpdateState() - Can't set state to ERROR or SUCCESS as a Masternode. \n");
            return;
        }

        LogPrintf("CMiragePool::UpdateState() == %d | %d \n", state, newState);
        if (state != newState) {
    ...
    /// Passively run Mirage in the background according to the configuration in settings (only for QT)
    bool DoAutomaticDenominating(bool fDryRun = false);
    bool PrepareMirageDenominate();

    /// Check for process in Mirage
    void Check();
    ...
    void SendMirageDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, CAmount amount);
    /// Get Masternode updates about the progress of Mirage
    ...
    /// Get the denominations for a specific amount of yup.
